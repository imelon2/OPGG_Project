  ...
  const _getGameData = (event: any) => {
    ...
    DataApi.dataAPI().then(function (data: any[]) {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        // 챔프 KDA 구별
        let _champData = {
          gameId: data[10].gameId,
          participant: "",
          gameCreation: data[10].gameCreation,
          champ: data[i].championId,
          title: "",
          win: data[i].win,
          champK: data[i].kills,
          champD: data[i].deaths,
          champA: data[i].assists,
          gold: data[i].goldEarned,
          totalDamage: data[i].totalDamageDealtToChampions,
        };
      ...
      let barMax =
        Math.ceil(maxNum / Math.pow(10, num.length - 1)) *
        Math.pow(10, num.length - 1);
