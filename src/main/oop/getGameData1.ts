    ...
    success: function (response) {
      ...
      for (let i = 0; i < response.teams[0].bans.length; i++) {
        let _ban1 = {
          teamBanChampId: response.teams[0].bans[i].championId
            ? response.teams[0].bans[i].championId
            : "noBan",
        };
      ...
      for (let i = 0; i < response.teams[1].bans.length; i++) {
        let _ban2 = {
          teamBanChampId: response.teams[1].bans[i].championId
            ? response.teams[1].bans[i].championId
            : "noBan",
        };
      ...
      for (let i = 0; i < 10; i++) {
        let totalMinionsKilled =
          response.participants[i].stats.totalMinionsKilled;
        let neutralMinionsKilled =
          response.participants[i].stats.neutralMinionsKilled;
        let data = {
          ...
          perk: response.participants[i].stats.perk0,
          subperk: response.participants[i].stats.perkSubStyle,
